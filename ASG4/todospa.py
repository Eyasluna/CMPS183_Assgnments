...
def db_to_view_task(row):
    ...
    id, d, s, pd, dd = row
    return {'taskid': id, 'taskdescription': d, 'status': db_to_view_status(s), 'post_date': pd, 'due_date': dd}
...
def view_to_db_new_task(task):
    ...
    return (task['taskdescription'], view_to_db_status(task['status']), task['post_date'], task['due_date'])
...
def db_create_task(new_task):
    ...
    db = DaBa("todo")
    # construct query to retrieve rows from todo table
    query = "INSERT INTO todo (task, status, post_date, due_date) Values('{td}', {status}, '{post_date}', '{due_date}')"
    query = query.format(td=new_task[0], status=(new_task[1]), post_date=new_task[2], due_date=new_task[3])
    # execute query
    print("Query: " + query)
    db.que(query)
    result = db.new_id()
...
def db_update_task(id, taskdescription, status, post_date, due_date):
    db = DaBa("todo")
    db.que("UPDATE todo SET task = ?, status = ?, post_date = ?, due_date = ? WHERE id = ?", (taskdescription, status, post_date, due_date, id))
    db.close()
...
def view_delete_task(vm_task_data):
    taskid = vm_task_data['taskid']
    db_delete_task(taskid)
    return {'ec': 200, 'em': 'ok'}

def view_update_task(vm_task_data):
    taskid = vm_task_data['taskid']
    vm_status = vm_task_data['status']
    vm_description = vm_task_data['taskdescription']
    vm_post_date = vm_task_data['post_date']
    vm_due_date = vm_task_data['due_date']
    db_status = view_to_db_status(vm_status)
    db_update_task(taskid, vm_description, db_status, vm_post_date, vm_due_date)
    task = view_get_task(taskid)
    if vm_description == task['taskdescription'] and vm_status == task['status']:
        return {'taskid': taskid, 'taskdescription': vm_description, 'status': vm_status, 'post_date': vm_post_date, 'due_date': vm_due_date}
    else:
...
def update_task():
    ...
    updated_task = view_update_task(task_data)
    print(updated_task)
...
def delete_task():
    ''' delete a task '''
    del_task = unpack_task(request)
    delete_msg = view_delete_task(del_task)
    return pack_task(delete_msg)
